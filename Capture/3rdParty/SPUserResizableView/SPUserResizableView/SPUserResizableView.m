...
@synthesize  minWidth, minHeight, preventsPositionOutsideSuperview;
...
- (void)setContentView:(UIView *)newContentView {
    ...
    _cv = newContentView;
    _cv.frame = CGRectInset(self.bounds, kSPUserResizableViewGlobalInset + kSPUserResizableViewInteractiveBorderSize/2, kSPUserResizableViewGlobalInset + kSPUserResizableViewInteractiveBorderSize/2);
    [self addSubview:_cv];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(userResizableViewDidBeginEditing:)]) {
        [_delegate userResizableViewDidBeginEditing:self];
    }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(userResizableViewDidEndEditing:)]) {
        [_delegate userResizableViewDidEndEditing:self];
    }
}
...
- (void)resizeUsingTouchLocation:(CGPoint)touchPoint {
    ...
    // (3b) Make it 16x9
    if (self.enforce16x19AspectRatio) {
        if (deltaW != 0) {
            newHeight = newWidth / (16.0 / 9.0);
        } else {
            newWidth = newHeight * (16.0 / 9.0);
        }
    }
