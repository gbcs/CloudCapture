...
@interface SoundBibleViewController ()

@end

@implementation SoundBibleViewController {
...
{
    [super viewDidLoad];
    self.navigationItem.title = @"SoundBible.com";
    
    [webview loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://www.soundbible.com"]]];
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    if (!detailView) {
        ...
        l.numberOfLines = 0;
        l.text = @"Select the 'WAV' or 'MP3' button to download a sound to your app library.\n\nPlease read the About page before using sounds from this library.";
        l.tag = 1;
        ...
        [faq setTitle:@"About" forState:UIControlStateNormal];
...
-(void)userTappedInfoButton:(id)sender {
    ...
    if (b.tag == 0) {
        [webview loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://soundbible.com/about.php"]]];
    }
...
-(void)downloadFile:(NSURLRequest *)request ofType:(NSString *)t {
    ...
    task = [session downloadTaskWithRequest:request completionHandler:^(NSURL *location, NSURLResponse *response, NSError *error) {
        if (!error) {
            NSString *mp3File = [[[UtilityBag bag] docsPath] stringByAppendingPathComponent:[[UtilityBag bag] pathForNewResourceWithExtension:t]];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    NSString *mp3 = @"type=mp3";
    NSString *wav = @"type=wav";
    
        // NSLog(@"str:%@", urlStr);
    
    if ([[[urlStr lowercaseString] substringWithRange:NSMakeRange([urlStr length] - [mp3 length], [mp3 length])] isEqualToString:mp3]) {
        [self downloadFile:request ofType:@"mp3"];
        return NO;
    } else if ([[[urlStr lowercaseString] substringWithRange:NSMakeRange([urlStr length] - [wav length], [mp3 length])] isEqualToString:wav]) {
        [self downloadFile:request ofType:@"wav"];
        return NO;
    }
