//
//  AddVideoVidevoViewController.m
//  Capture
//
...
@interface AddVideoVidevoViewController ()

@end

@implementation AddVideoVidevoViewController {
...
-(void)cleanup {
    NSLog(@"Addwww.videvo.netViewController dealloc");
...
{
    [super viewDidLoad];
    self.navigationItem.title = @"www.videvo.net";
    self.navigationController.toolbarHidden = NO;
    self.navigationController.navigationBar.barStyle = UIBarStyleBlackTranslucent;
    [webview loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://www.videvo.net/"]]];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    NSLog(@"url:%@", urlStr);
     ...
     //https://www.youtube.com/embed/?status=ok&hl=en&allow_embed=0&ps=docs&partnerid=30&autoplay=0&docid=0By6JurPqWRsyQndXc1hmTldQZXc&abd=0&el=leaf&title=TigerCub_CaboSanLucas_ChargeCustomers_To_Hold&iurl=https%3A%2F%2Fdocs.google.com%2Fvt%3Fid%3D0By6JurPqWRsyQndXc1hmTldQZXc&token=1&plid=V0QTVjvjjd3Cxw&timestamp=1393366982620&length_seconds=10&enablejsapi=1&origin=https%3A%2F%2Fdocs.google.com
    
    NSString *prefix = @"https://www.youtube.com/embed/";
    NSInteger prefixLen = [prefix length];
    
    if (([urlStr length] > prefixLen) && ([[urlStr substringToIndex:[prefix length]] isEqualToString:prefix])) {
        if ([urlStr hasSuffix:@"docs.google.com"]) {
            ...
            NSLog(@"download:%@ title:%@", [mdQueryStrings objectForKey:@"iurl"], [mdQueryStrings objectForKey:@"title"]);
