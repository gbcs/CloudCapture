//
//  AudioMergeConfirmViewController.m
//  Capture
//
...
@interface AudioMergeConfirmViewController ()
...
@implementation AudioMergeConfirmViewController
...
-(void)cutClip {
    ...
    if(error) {
        ...
        //  NSMutableArray *audioMix = [[NSMutableArray alloc] initWithCapacity:[_mergeList count]];

    for (NSInteger x=0;x<[_mergeList count];x++) {
        CMTime start = [[[_mergeList objectAtIndex:x] objectAtIndex:0] CMTimeValue];
            //  CGFloat volume = [[[_mergeList objectAtIndex:x] objectAtIndex:2] floatValue];
            //  NSInteger transitionIn = [[[_mergeList objectAtIndex:x] objectAtIndex:3] integerValue];
            // NSInteger transitionOut = [[[_mergeList objectAtIndex:x] objectAtIndex:4] integerValue];
            //  CMTime duration = CMTimeMakeWithSeconds([[[_mergeList objectAtIndex:x] objectAtIndex:5] floatValue], _clip.duration.timescale);
        AVURLAsset *asset = [[_mergeList objectAtIndex:x] objectAtIndex:6];
        AVMutableCompositionTrack *addTrack = [mutableComposition addMutableTrackWithMediaType:AVMediaTypeAudio preferredTrackID:kCMPersistentTrackID_Invalid];
        
        AVAssetTrack *addTrackAsset = [[asset tracksWithMediaType:AVMediaTypeAudio] objectAtIndex:0];
        [addTrack insertTimeRange:CMTimeRangeFromTimeToTime(kCMTimeZero, asset.duration) ofTrack:addTrackAsset atTime:start error:&editError];
    }
    ...
    item.value = [NSString stringWithFormat:@"%@[insert]", originalTitle];
    [metadata addObject:item];
...
-(void)userTappedCommit {
    ...
    [[UtilityBag bag] logEvent:@"mergeAudio" withParameters:nil];
