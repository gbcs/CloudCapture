//
//  FreeSoundViewController.m
//  Capture
//
//  Created by Gary Barnett on 1/18/14.
//  Copyright (c) 2014 Gary Barnett. All rights reserved.
//

#import "FreeSoundViewController.h"

@interface FreeSoundViewController ()

@end

@implementation FreeSoundViewController{
...
{
    [super viewDidLoad];
    self.navigationItem.title = @"freeSound.org";
    
    [webview loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://www.freesound.org"]]];
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    if (!detailView) {
        ...
        l.numberOfLines = 0;
        l.text = @"Select the 'Download' button to download a sound to your app library.\n\nPlease review the FAQ before using sounds from this library.";
        l.tag = 1;
...
-(void)userTappedInfoButton:(id)sender {
    ...
    if (b.tag == 0) {
        [webview loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://freesound.org/help/faq/"]]];
    }
...
-(void)downloadFile:(NSURLRequest *)request {
    ...
    task = [session downloadTaskWithRequest:request completionHandler:^(NSURL *location, NSURLResponse *response, NSError *error) {
        if (!error) {
            NSString *mp3File = [[[UtilityBag bag] docsPath] stringByAppendingPathComponent:[[UtilityBag bag] pathForNewResourceWithExtension:ext]];
