//
//  AddPhotoPublicDomainPhotosViewController.m
//  Capture
//
...
@interface AddPhotoPublicDomainPhotosViewController ()

@end

@implementation AddPhotoPublicDomainPhotosViewController {
...
-(void)cleanup {
    NSLog(@"AddPhotoPublicDomainPhotosViewController dealloc");
...
{
    [super viewDidLoad];
    self.navigationItem.title = @"commons.wikimedia.org";
    
    [webview loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://commons.wikimedia.org/wiki/Main_Page"]]];
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    if (!detailView) {
        ...
        l.numberOfLines = 0;
        l.text = @"Select the 'Original file' link under an image to download a photo your camera roll.\n\nPlease read the Reuse page before using photos from this library.";
        l.tag = 1;
        ...
        [faq setTitle:@"Reuse" forState:UIControlStateNormal];
...
-(void)userTappedInfoButton:(id)sender {
    ...
    if (b.tag == 0) {
        [webview loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://commons.wikimedia.org/wiki/Commons:Reusing_content_outside_Wikimedia"]]];
    }
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    NSString *prefix = @"http://upload.wikimedia.org/wikipedia/commons";
