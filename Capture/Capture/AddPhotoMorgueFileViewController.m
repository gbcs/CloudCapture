//
//  AddPhotoMorgueFileViewController.m
//  Capture
//
...
@interface AddPhotoMorgueFileViewController ()

@end

@implementation AddPhotoMorgueFileViewController {
...
-(void)cleanup {
    NSLog(@"AddPhotoMorgueFileViewController dealloc");
...
{
    [super viewDidLoad];
    self.navigationItem.title = @"morguefile.com";
    
    [webview loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://www.morguefile.com"]]];
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    if (!detailView) {
        ...
        l.numberOfLines = 0;
        l.text = @"Select the 'Download Image' button to download the photo to your camera roll.\n\nPlease review the License before using this library.";
        l.tag = 1;
...
-(void)userTappedInfoButton:(id)sender {
    ...
    if (b.tag == 0) {
        [webview loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://morguefile.com/license/morguefile"]]];
    }
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    NSString *prefix = @"http://cdn.morguefile.com/imageData/public/files/";
